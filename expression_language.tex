...
T(x) &= 
 ...
 \end{align*}
 We want to show that the denotation of this expression has the same behavior as the random variable described by Karp. First, let $F = \lambda f. \lambda x. (\text{if } x<1 \text{ then } 0 \texttt{ else } 1) + f (p * x)$ and $a = \texttt{if } x<1 \texttt{ then } 0 \texttt{ else } 1$, and see that 
 \begin{align*}
