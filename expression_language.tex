...
T(n) &= 1 + T(pn), \ 0 < p < 1
\end{align*} 
Consider that this recurrence only makes sense if its input is in the set $\{x \in \R \mid \exists k \in \N \text{ s.t. } p^kx = 1\}$; we will assume that this set is the domain of $T$. 
 See, then, that $T(n) = \log_{p}1/n $
...
Then $\llbracket \texttt{T} \rrbracket = \lim_{n \to \infty}\{F^n (\perp)\}$ is defined for all $x \in \R$ such that $p^kx = 1$
for some $k \in \N$ as
\begin{align*}
\lim_{n \to \infty}\{F^n (\perp)\}(x) = k = \log_{p}1/x
\end{align*}
Then $\llbracket \texttt{T} \rrbracket(x) = \log_{p}1/x= T(x)$. Thus, $\llbracket \texttt{T} \rrbracket = T$.
