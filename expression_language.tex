In section 1, Karp defines recurrences of the form $T(x) = a(x) + T(m(x))$. In section 2, however, he offers example
applications of his theorems, using recurrences with initial conditions. For example, a recurrence for a randomized list ranking 
algorithmvis written as
\begin{align*}
T(1) &= 1 \\
T(n) &= 1 + T(\frac{3}{4}n)
\end{align*}
These examples seem to be in conflict with section 1, where Karp never discusses initial conditions to recurrences. Indeed, 
there is no clear way to write the above equations as one equation of the form $T(x) = a(x) + T(m(x))$.

We define an expression language which will allow us to write and type-check the recurrence relations offered in section 2. 
The syntax of this language is mostly analogous to PCF, with one key exception: instead of a 
...
\section{Examples}
We may use this syntax to express the algorithms discussed in section 2, where Karp offers both a general recurrence and an
initial condition. For instance, consider the following recurrence:
\begin{align*}
T(1) &= 1 \\
T(n) &= 1 + T(pn), \ 0 < p < 1
\end{align*} 
...
\end{align*}
Where $n \in \N$ is the minimum number such that $p^nx<1$. Then $\llbracket \texttt{T} \rrbracket(x) = 1 + \left \lfloor{\log_{p}1/n}\right \rfloor = T(x)$. Therefore, $\llbracket \text{T} \rrbracket = T$.
