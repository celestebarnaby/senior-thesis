...
T(n) &= 1 + T(pn), \ 0 < p < 1
\end{align*} 
Consider that this recurrence only makes sense if its input is in the set $\{x \in \R \mid \exists k \in \N \text{ s.t. } p^kx = 1\}$; we will assume that this is the domain of $T$. 
 See, then, that for all $ x \in \{x \in \R \mid \exists k \in \N \text{ s.t. } p^kx = 1\}, \ T(x) = \log_{p}1/x $
...
\texttt{T} &= \texttt{fix}(\lambda f. \lambda x.\ifelse{x=1}{0}{1 + f(p*x)})
 ...
 First, let 
 \begin{align*}
 \texttt{F} =\lambda f. \lambda x.\ifelse{x=1}{0}{1 + f(p*x)},
 \end{align*}
...
\end{align*}
Where $p^kx=1$. Then $k = \log_{p}1/x$, so
 $\llbracket \texttt{T} \rrbracket(x) = \log_{p}1/x= T(x)$. Thus, $\llbracket \texttt{T} \rrbracket = T$.
