...
\ndComment{Subtraction and division are not continuous.  Try to find other
operations that are sufficient for defining typical recurrences.  
Also check the numeric relations.}

\ndComment{What about term constructors for product types?}

\ndComment{Do we need values at all, since we are interested in the
equational calculus?}

\ndComment{It is probably best to put the syntax, type system, and
semantics into figures.  That makes the formatting a bit cleaner.}
...
\ndComment{Maybe an example here would be useful.  Show how we would informally
write a recurrence, and how that recurrence would be expressed in this
language.}
...
\ndComment{You should make sure to define the arithmetic operations on
$+\infty$.}
\ndComment{Should $\bot$ in the semantics of relational operators be
$+\infty$?}
 \begin{align*}
...
\ndComment{You have $p*x$ instead of $m(x)$ in the second equation.}
 \begin{align*}
...
\ndComment{%
$F^n(\bot)(d) = \llbracket\texttt{a}(x) + f(\texttt{m}(x))\rrbracket\{\dotsc\}$.}
 \begin{align*}
...
Therefore, 
\ndComment{%
Remember, $\tmden T{}$ is a semantic function, so you are
describing $\tmden T{}(d)$.}
\begin{thm}
\begin{align*}
