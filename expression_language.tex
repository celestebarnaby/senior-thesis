...
Consider that we may express the general equation T(x) = a(x) + T(m(x)) in our expression language as follows:
 ...
 \begin{align*}
 \llbracket \texttt{T} \rrbracket &= \underline{\text{fix}}(\llbracket \texttt{F} \rrbracket) \\
 ...
 \\\text{By the CPO fixpoint theorem, } \underline{\text{fix}}(\llbracket \texttt{F} \rrbracket) &= \underline{\text{fix}}(F) \\
...
$\texttt{a}(x) = \emph{if } x<1 \emph{ then } 0 \emph{ else } 1$ and $\texttt{m}(x) = p*x$:
 \begin{align*}
 \texttt{fix}(\lambda f. \lambda x. (\emph{if } x<1 \emph{ then } 0 \emph{ else } 1) + f (p * x))
 \end{align*}
 Then, by theorem 6,
 \begin{align*}
