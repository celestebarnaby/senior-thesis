...
&= \lim_{n \to \infty}\{g^n (\perp)\}
 ...
 \rightarrow \texttt{real}\rrbracket$ is the function $f_{\perp}$ that maps every element in $\llbracket \texttt{real} \rrbracket$ to 
 $0$. 
 \begin{align*}
 \forall d \in \R_{[0,+\infty]}, \ g(f_{\perp})(d) &= \llbracket \lambda x.a + f(p*x)\rrbracket\{f \mapsto f_{\perp}, x \mapsto d\} \\
 &= \llbracket a \rrbracket(d) + f_{\perp}(pd) \\
 &= \llbracket a \rrbracket(d)  \\
 \forall d \in \R_{[0,+\infty]}, \ g(g(f_{\perp}))(d) &= \llbracket \lambda x.a + f(p*x)\rrbracket\{f \mapsto g(f_{\perp}),x\mapsto d\} \\
 &= \llbracket a \rrbracket(d) + g(f_\perp)(pd) \\
 &= \llbracket a \rrbracket(d) + \llbracket \lambda x.a + f(p * x)\rrbracket\{f\mapsto f_\perp, x\mapsto pd\} \\
 &= \llbracket a \rrbracket(d) + \llbracket a \rrbracket(pd) + f_\perp(p^2d) \\
 &= \llbracket a \rrbracket(d) + \llbracket a \rrbracket(pd) \\ 
 &\ldots \\
 \forall d \in \R_{[0,+\infty]}, \ g^n(f_{\perp})(d) &= \sum_{i=0}^{n-1} \llbracket a \rrbracket (p^{i}d) \\
 \lim_{n \to \infty}\{g^n (f_{\perp})\}(d) &= \sum_{i=0}^{\infty} \llbracket a \rrbracket (p^{i}d) \\
 \end{align*}
