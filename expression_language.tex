...
in section 2 of Karp's paper. The syntax of this language is mostly analogous to Programming Computable Functions (PCF) ---a typed functional language with a fixed-point combinator--- with one exception: instead of a 
...
\end{align*}
 ...
 is a function $f: \llbracket \tau \rrbracket \rightarrow \llbracket \tau'  \rrbracket$ such that for 
...
\section{Example}
We will look at a recurrence Karp offers in this section as an example, to check whether recurrences in our expression
...
Then $\llbracket \texttt{T} \rrbracket = lim_{n \to \infty}\{F^n (\perp)\}$ is defined for $\forall x \in \{x \in \R \mid \exists k \in \N \text{ s.t. } p^kx = 1\}$ as
\begin{align*}
\lim_{n \to \infty}\{F^n (\perp)\}(x) = n - 1
\end{align*}
Where $n \in \N$ is the minimum number such that $p^nx<1$. Then $\llbracket \texttt{T} \rrbracket(x) = 1 + \log_{p}1/n= T(x)$. Therefore, $\llbracket \texttt{T} \rrbracket = T$.
