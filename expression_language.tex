...
Just as with code, format your \LaTeX\ source well.  Use blank lines and
shorter lines so that it is easy to navigate the source code by
jumping to the next blank line, etc.  It also makes section breaks stand
out in the \LaTeX\ source, just like using blank lines helps code blocks
stand apart from each other.

Also, let's reorganize the document to use a galley file.  A galley file
consists of your preamble and \verb!\include! statements, each of which
includes a file.  Then write each chapter in its own file.  So the 
overall organization looks something like:
\begin{itemize}
\item \verb!thesis.tex! (the galley file):
\begin{verbatim}
...
% Chapters
\include{intro}
\include{deterministic_recs}
...
\end{document}
\end{verbatim}

\item \verb!intro.tex!:  your introduction.
\item \verb!deterministic_recs.tex!:  the contents of this file.
\item Etc.
\end{itemize}
You then run \verb!latex! on \verb!thesis.tex!.  Of course, you can name
your chapter files however you want.
\end{ndBlockComment}

\section{Expression Language}
\ndComment{You are talking about deterministic recurrences in this section.}
...
\cprotect\ndComment{Use display math such as \verb!\[...\]!
or the \texttt{align*} environment for displayed mathematics.  Don't use in-line
math mode like \verb!$...$! and explicit linebreaks.  As an example, I've
rewritten your syntactic category definitions with an \verb!array!, which
lets me align the $::=$ symbol, as well as align the right-hand sides for
multi-line definitions.  Also, \verb!\mid! is the binary relation symbol
that is a vertical bar; using it ensures the appropriate spacing.  Use
macros for constructs like $if\dots then\dots else$.  Also use consistent fonts,
like typewriter font for all expressions.}
...
\ndComment{Make sure to tell the reader what a $\Gamma$ is.  You are missing 
...
\ndComment{We don't need an operational semantics.}
...
\ndComment{You need to specify the order on product and function spaces
somewhere.}
...
\ndComment{Tell the reader what kind of thing $\eta$ is.}
\ndComment{Probably drop the underline on the semantic fix function.}
 \begin{align*}
...
\ndComment{Both inductive cases need a bit more argument.  For product spaces,
you need to show that $(\bigvee C_1,\bigvee C_2)$ is in fact the least
upper bound on~$C$ by showing that it is an upper bound and that it is 
no larger than any other upper bound.}
\begin{itemize}
...
&= \bigvee\{\llbracket \Gamma \vdash \texttt{fix}(\lambda f \lambda x.e)\rrbracket\eta_i\}^{\infty}_{i=1}
 ...
 \begin{proof} 
\cprotect\ndComment{There is no longer any Lemma~1.  
\LaTeX\ has a cross-referencing
mechanism for this situation.  Use \verb!\label{blah}! in the environment
...
I've used \verb!\label! and \verb!\ref! here as an example.}
 By reasoning similar to Lemma 1 (i.e., Lemma~\ref{lem:join-interchange-1}).
 \end{proof}
...
\ndComment{You've got some minor indexing errors here.}
 \begin{align*}
