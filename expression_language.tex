...
&= \bigvee_i \llbracket \Gamma \vdash \texttt{fix}(\lambda f \lambda x.e)\rrbracket\eta\{x\mapsto a_i\}
 ...
 Suppose, now, that 
 \begin{align*}
 &\bigvee_j(\bigvee_i((F_i)^j \perp)) < \bigvee)i(\bigvee_j((F_i)^j \perp))\\ 
...
T(n) &= 1 + T(pn), \ 0 < p < 1
\end{align*} 
Consider that this recurrence only makes sense if we assume that there can be no input may be smaller than 1,
and if for any input $n$, there exists a $k \in \N$ such that $p^kn = 1$.
For $\forall n > 1, \ \exists k \in \N$ such that $p^kn \leq1, p^{k-1}n > 1$. See, then, that $T(n) = 1 + \left \lfloor{\log_{p}1/n}\right \rfloor$. CHANGE STUFF HERE
