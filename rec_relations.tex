...
$h_2(x)(l)$. The arguments to these random variables, then, should be the left and right sublists themselves. 
 ...
 complications and ambiguities, which Karp never addresses directly. We define the following syntax
 to write and type-check these recurrences. This syntax is based on $\lambda$LF---it contains terms, types and kinds,
 which allow us to distinguish between proper types and type families.
 
 \[
\begin{array}{rcl}
\tau &::=& X \mid \texttt{real} \mid \texttt{nat} \mid \texttt{bool} \mid \tau \times \tau \mid \tau \rightarrow \tau \mid \Pi x:\tau.\tau
\mid \tau \ e \\
k &::=& * \mid \Pi x : \tau.k \mid \texttt{list(n)}  \\
