...
\mid \tau \ e \mid \texttt{list} \\
k &::=& * \mid \tau \rightarrow k \\
...
We use the constant $\texttt{list}$ to denote the type family of kind
$\Pi n:\texttt{nat}.*$.
The syntax consists of entities of three levels: expressions, types, and kinds. The inclusion of kinds lets us
differentiate between proper types, $*$, and type families $\tau \rightarrow k$. 

\begin{figure}
\[
\begin{array}{c}
\Gamma \vdash * \\ \\
\dfrac{\Gamma \vdash \tau :: * \ \ \ \Gamma \vdash k}{\Gamma \vdash \tau \rightarrow k} \\ \\
...
\dfrac{\Gamma \vdash \tau :: \sigma \rightarrow k \ \ \ \Gamma \vdash e : \sigma}{\Gamma \vdash \tau \ e :: k} \\ \\
\end{array}
\]
\caption{Well-formed kinds and kind judgements for dependent type language.}
\end{figure}
...
\begin{figure}
 \begin{align*}
\llbracket * \rrbracket &= \bigcup_{k \in \N} A_k \\
\text{Where $A_k$ is defined inductively as follows:} \\
A_0 &= \{\R_{\perp}, \ \N_{\perp}, \ \{true, \ false\}_{\perp}\} \\
A_{k+1} &= A_k \cup \{ X \times Y \mid X, Y \in A_k\} \cup \\
\{ Y \rightarrow X \mid Y \in A_k, \ \exists X_i &\in A_k \text{ for } i \in Y \text{ s.t. } X = \bigcup_{i \in Y} X_i\} \\
\llbracket \tau \rightarrow k \rrbracket\eta &= \{f : \llbracket \tau \rrbracket\eta \rightarrow \llbracket k \rrbracket\eta\}\\ \\
...
\llbracket \texttt{nat} \rrbracket &= \N_{\perp} \\
 ...
 \llbracket \tau \ e \rrbracket\eta &= \llbracket \tau \rrbracket\eta( \llbracket e \rrbracket\eta) \\
 \llbracket \texttt{list} \rrbracket &= \llbracket \texttt{nat} \rightarrow * \rrbracket  \\
 %
 \end{align*}
 \caption{Denotational semantics for types and kinds.}
 \end{figure}
 \begin{figure}
 \begin{align*}
...
\begin{proof}
\begin{itemize}
\item $\tau = (\tau' :: \sigma \rightarrow k)(e : \sigma) $ \\
Then $\llbracket \tau \rrbracket\eta = \llbracket \tau \rrbracket\eta(\llbracket e \rrbracket\eta)$. Then, by inductive hypothesis, $\llbracket \tau \rrbracket$ is a CPO.
\end{itemize}
\end{proof}
