...
\begin{itemize}
\item $\tau = \texttt{nat}$ \\
Then $\llbracket \tau \rrbracket$ is flat-ordered, so it is a CPO.
\item $\tau = \lst(e)$ \\
Then $e : \texttt{nat}$ and $\llbracket \tau \rrbracket = \Omega_{\llbracket e \rrbracket}$. This set is flat-ordered,
so $\llbracket \tau \rrbracket$ is a CPO.
\item $\tau = \Pi x:\tau_1.\tau_2$ \\
Then $\llbracket \tau \rrbracket =  \{f: \llbracket \tau_1 \rrbracket\eta \rightarrow 
 ...
 f(a) \in \llbracket \tau_2 \rrbracket\eta\{x \mapsto a' \}$, for some $a' \in \llbracket \tau_1 \rrbracket\eta$.  \\ \\
 %
